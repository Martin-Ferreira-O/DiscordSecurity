...
export default class HelpCommand extends CommandBase {
	...
	async run(bot: Bot, message: Message, args: Array<string>) {
		...
		if (!args.length) {
			const user = [...bot.commands.values()].filter(
				(v: CommandBase) => v.category === 'user'
			);
			const admin = [...bot.commands.values()].filter(
				(v: CommandBase) => v.category === 'Admin'
			);
			embed.addFields(
				{
					name: 'User',
		...
		if (command) {
			embed.setDescription(
				lang.commandInfo.replace('%command%', command.name)
			);
			embed.addFields(
				{
					name: 'Name',
		...
		embed.setDescription(
			lang.commandNotFound.replace('%command%', args[0])
		);
		return message.reply({ embeds: [embed] });
	}
}
